...
# from finrl.config import TRAINED_MODEL_DIR, TENSORBOARD_LOG_DIR
...
class DRLAgent:
    ...
    def DRL_prediction(model, environment, deterministic=True):
        ...
        for i in range(len(environment.df.index.unique())):
            ...
            if i == (len(environment.df.index.unique()) - 2):
                account_memory = test_env.env_method(method_name="save_asset_memory")
                actions_memory = test_env.env_method(method_name="save_action_memory")
                rewards_memory = test_env.env_method(method_name="save_rewards_memory")
            #   state_memory=test_env.env_method(method_name="save_state_memory") 
            #   add current state to state memory
            if dones[0]:
                print("hit end!")
                break
        return account_memory[0], actions_memory[0] , rewards_memory[0]
    ...
    #     episode_total_assets = [environment.asset_memory[0]]
    #     done = False
    #     while not done:
    ...
    #         episode_return = total_asset / environment.asset_memory[0]
    #         episode_returns.append(episode_return)

    #     print("episode_return", episode_return)
    #     print("Test Finished!")
    #     return episode_total_assets , action
...
##        )
##        model.save(
##            f"{os.getcwd()}"+"/trained_models/"+f"{model_name.upper()}_{total_timesteps // 1000}k_{iter_num}"
##        )
##        return model
##
##    @staticmethod
